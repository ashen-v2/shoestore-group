...
from models.reviews import Review, ReviewRead, ReviewUpdate, ReviewReadWithProduct
...
@router.get("/products/{product_id}", response_model=list[ReviewReadWithProduct])
def get_product_reviews(product_id : int, session : Session = Depends(get_session), limit : int = 50, skip : int = 0):
    ...
    product_reviews = session.exec(select(Review, User).join(User,Review.user_id == User.id).where(Review.product_id == product_id, Review.is_reviewed == True )).all()
    product_reviews_out : list[ReviewReadWithProduct] = []
    for product_review, product_user in product_reviews:
        updated_product_review : ReviewReadWithProduct = ReviewReadWithProduct.model_validate(product_review)
        updated_product_review.user_name = product_user.name
        updated_product_review.profile_image_url = product_user.profile_image_url
        product_reviews_out.append(updated_product_review)
    return product_reviews_out
